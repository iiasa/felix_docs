...
$$Imp\_CC\_on\_GWP$$ is related to the climate change damage (details see the section **2.2 Climate impacts on GWP**), and $$Imp\_Biodiv\_on\_GWP$$ is an endogenous variable related to the mean species abundance ($$MSA$$) from the biodiversity module (the section [6. Biodiversity](1_1_6_biodiversity.md)). The economic cost of biodiversity loss ($$Imp\_Biodiv\_on\_GWP$$) is deeply uncertain, and FeliX includes this function not to accurately quantify the loss but to explore this uncertainty space. The total $$REO$$ is the sum of the $$REO$$ generated by the labor force by skill ($$REO_{skill}$$). As mentioned in **1.5 Labor force**, skill of labor force is distinguished into skilled and unskilled. $$REO_{skill}$$ are computed based on a Cobb‑Douglas production function, depending on the technology ($$\varphi\_Tech_{skill}$$) and capital ($$\varphi\_K_{skill}$$) allocated to the labor force, and the size of the labor force input.
